...
if results is not None:
    ...
    # You'll need to pass the original data used in analysis
    # Let's assume you have it available
    data_used_in_analysis = merged_data
    report = report_generator.generate_pls_report(results,data_used_in_analysis, "pls_analysis_report")
...
def print_report_summary(results):
    ...
    print(f"Observations: {results['X'].shape[0]}")
    print(f"Predictors: {len(results['predictors'])}")
    print("\nPerformance Summary:")
    for hormone, r2 in results['r2_scores'].items():
        print(f"  {hormone}: R² = {r2:.3f}")

    print("\nTop 5 Predictors:")
    for i, (idx, row) in enumerate(results['importance_df'].head(5).iterrows()):
        print(f"  {i + 1}. {row['feature']} (VIP: {row['VIP_score']:.3f})")