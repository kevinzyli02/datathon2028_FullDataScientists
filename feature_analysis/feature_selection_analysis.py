...
RANDOM_STATE = 42
SAMPLE_SIZE = 15000
USE_TOP_FEATURES = False  # Set to True to use only top 15 features
TOP_FEATURES_COUNT = 15  # Number of top features to use
...
# =============================================================================
# ENHANCEMENT 1: BASELINE NORMALIZATION WITH MENSTRUATION PHASE
# =============================================================================

def normalize_by_menstruation_baseline(df, targets, baseline_days=3):
    """Normalize hormone levels by patient-specific baseline using first days of menstruation phase"""
    ...
    for target in targets:
        ...
        for patient_id in df['id'].unique():
            ...
            # Find menstruation phase data - check for common menstruation phase labels
            menstruation_phrases = ['menstruation', 'menstrual', 'period', 'bleeding']
            menstruation_data = None

            for phrase in menstruation_phrases:
                if 'phase' in patient_data.columns:
                    menstruation_mask = patient_data['phase'].str.lower().str.contains(phrase, na=False)
                    if menstruation_mask.any():
                        menstruation_data = patient_data[menstruation_mask]
                        print(f"   Patient {patient_id}: Found menstruation data using '{phrase}'")
                        break

            if menstruation_data is None or len(menstruation_data) == 0:
                print(f"   ⚠️ Patient {patient_id}: No menstruation phase data found, using first {baseline_days} days")
                # Fallback: use first baseline_days of study
                baseline_data = patient_data.head(baseline_days)
            else:
                # Use first baseline_days of menstruation phase
                baseline_data = menstruation_data.head(baseline_days)
                print(f"   Patient {patient_id}: Using first {len(baseline_data)} menstruation days for baseline")
            ...
            if not np.isnan(baseline_value) and baseline_value > 0:
                ...
                print(
                    f"   Patient {patient_id}: baseline {target} = {baseline_value:.2f} (from {len(baseline_data)} menstruation days)")
            else:
                print(f"   ⚠️ Patient {patient_id}: Invalid baseline for {target}")
...
def add_cycle_phase_features(df):
    ...
    df['phase_menstruation'] = ((df['cycle_day'] >= 1) & (df['cycle_day'] <= 7)).astype(int)
...
def load_comprehensive_data(files, data_dir, sample_size=SAMPLE_SIZE):
    """Load and merge comprehensive dataset efficiently with memory optimization"""
    ...
    for file_name in files[1:]:
        ...
        try:
            ...
            # Use optimized reading for large files
            if file_name == 'sleep_score.csv':
                print(f"   📊 Reading large file {file_name} with optimized settings...")
            ...
            # Memory-efficient merge - sample large datasets before merging
            if len(new_data) > 50000:
                sample_size_new = min(20000, len(new_data))
                new_data = new_data.sample(n=sample_size_new, random_state=RANDOM_STATE)
                print(f"   🔽 Sampled new data to {new_data.shape}")

            # Use more efficient merge strategy
            if len(df) > 10000 and len(new_data) > 10000:
                print(f"   🔄 Using efficient merge strategy for large datasets...")
                # Filter both datasets to common keys before merge
                common_ids = set(df['id']).intersection(set(new_data['id']))
                if 'day_in_study' in df.columns and 'day_in_study' in new_data.columns:
                    df_filtered = df[df['id'].isin(common_ids)].copy()
                    new_data_filtered = new_data[new_data['id'].isin(common_ids)].copy()

                    # Merge on filtered data
                    df = pd.merge(df_filtered, new_data_filtered, on=merge_keys, how='left',
...
def handle_file_structure(df, file_name):
    ...
    if file_name in ['glucose.csv', 'wrist_temperature.csv', 'sleep_score.csv']:
...
def select_top_features(X_train, y_train, feature_names, n_features=TOP_FEATURES_COUNT):
    """Select top n features based on feature importance from RandomForest"""
...
# =============================================================================
# ENHANCED MODEL TRAINING WITH CONSISTENT SPLIT
# =============================================================================

def train_enhanced_models_consistent_split(df, feature_sets, targets, use_top_features=USE_TOP_FEATURES):
    """Enhanced training function with consistent 80/20 split across all models"""

    print("\n🚀 TRAINING ENHANCED MODELS WITH CONSISTENT 80/20 SPLIT")
    print("=" * 80)

    if use_top_features:
        print(f"🎯 USING TOP {TOP_FEATURES_COUNT} FEATURES ONLY")
    else:
    ...
    # Create a single consistent split for ALL targets
    print(f"\n🎯 CREATING CONSISTENT 80/20 SPLIT FOR ALL TARGETS")
    patient_ids = df['id'].unique()
    n_patients = len(patient_ids)

    # Split patient IDs once
    train_patients, test_patients = train_test_split(
        patient_ids,
        test_size=TEST_SIZE,
        random_state=RANDOM_STATE
    ...
    for target in targets:
        ...
        # Use the same patient split for all targets
        train_data = target_data[target_data['id'].isin(train_patients)]
        test_data = target_data[target_data['id'].isin(test_patients)]

        print(f"📊 Dataset for {target}:")
        print(f"   Training: {train_data.shape} ({len(train_patients)} patients)")
        print(f"   Testing:  {test_data.shape} ({len(test_patients)} patients)")
        ...
        if use_top_features and len(all_features) > TOP_FEATURES_COUNT:
            selected_features = select_top_features(X_train_imputed, y_train, all_features, TOP_FEATURES_COUNT)

            # Update feature sets
            all_features = selected_features
            X_train_imputed = imputer.fit_transform(train_data[all_features])
            X_test_imputed = imputer.transform(test_data[all_features])

            print(f"🔍 Using top {len(all_features)} features for {target}")
        ...
        for model_name, model in models.items():
            ...
            try:
                ...
                print(f"   Test R²: {test_metrics['correlation_metrics']['r2']:.4f}")
...
def create_comprehensive_report(results, feature_sets):
    ...
    with open(OUTPUT_DIR / "patient_wise_model_report.txt", 'w', encoding='utf-8') as f:
        f.write("COMPREHENSIVE MODEL ANALYSIS - CONSISTENT 80/20 PATIENT-WISE SPLIT\n")
        f.write("=" * 80 + "\n\n")
        f.write("MODEL EVALUATION WITH CONSISTENT TRAIN/TEST SPLIT ACROSS ALL TARGETS\n")
        f.write(f"Sample Size: {SAMPLE_SIZE:,}\n")
        f.write(f"Test Size: {TEST_SIZE * 100}% of PATIENTS\n")
        f.write(f"Random State: {RANDOM_STATE}\n")
        f.write(f"Top Features Only: {USE_TOP_FEATURES}\n")
        if USE_TOP_FEATURES:
            f.write(f"Top Features Count: {TOP_FEATURES_COUNT}\n")
        f.write("\n")

        # Show patient split information
        if results:
            first_target = list(results.keys())[0]
            if first_target in results and results[first_target]:
                first_model = list(results[first_target].keys())[0]
                if results[first_target][first_model]:
                    train_patients = results[first_target][first_model]['train_patients']
                    test_patients = results[first_target][first_model]['test_patients']
                    f.write(f"TRAINING PATIENTS: {len(train_patients)}\n")
                    f.write(f"TESTING PATIENTS: {len(test_patients)}\n")
                    f.write(f"Total Patients: {len(train_patients) + len(test_patients)}\n\n")
        ...
        if first_target in results and results[first_target]:
            ...
            if results[first_target][first_model]:
                ...
                patient_split_df = pd.DataFrame({
                    ...
                    'patient_id': list(train_patients) + list(test_patients)
                })
                patient_split_df.to_csv(OUTPUT_DIR / "patient_split_info.csv", index=False)
                print("✅ Patient split information saved as CSV")
...
# =============================================================================
# UPDATED MAIN EXECUTION WITH CONSISTENT SPLIT
# =============================================================================

def main_with_consistent_split():
    """Enhanced main function with consistent patient split across all targets"""

    print("🚀 ENHANCED PIPELINE WITH CONSISTENT 80/20 PATIENT SPLIT")
    print("=" * 80)

    print(f"🎯 CONFIGURATION:")
    print(f"   Use Top Features: {USE_TOP_FEATURES}")
    if USE_TOP_FEATURES:
        print(f"   Top Features Count: {TOP_FEATURES_COUNT}")
    ...
    try:
        ...
        # Step 2: Apply baseline normalization with MENSTRUATION PHASE
        print("\n🔄 STEP 2: APPLYING MENSTRUATION PHASE BASELINE NORMALIZATION...")
        if 'phase' in df.columns:
            df = normalize_by_menstruation_baseline(df, TARGETS, baseline_days=3)
            print("✅ Using menstruation phase for baseline calculation")
        else:
            print("⚠️ 'phase' column not found, using first 3 study days for baseline")
            df = normalize_by_menstruation_baseline(df, TARGETS, baseline_days=3)  # Falls back to first days
        ...
        # Step 5: Train enhanced models with consistent split
        print("\n🤖 STEP 4: TRAINING ENHANCED MODELS WITH CONSISTENT SPLIT...")
        results = train_enhanced_models_consistent_split(df, feature_sets, NORMALIZED_TARGETS,
                                                         use_top_features=USE_TOP_FEATURES)

        # Step 6: Create comprehensive report
        create_comprehensive_report(results, feature_sets)

        print(f"\n✅ ENHANCED PERSONALIZED MODELING WITH CONSISTENT SPLIT COMPLETE!")
...
if __name__ == "__main__":
    # Run full enhanced pipeline with consistent split
    main_with_consistent_split()